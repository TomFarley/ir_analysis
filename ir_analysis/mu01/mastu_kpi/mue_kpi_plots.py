...
def check_time_axis_rescale():
    ...
    plt.figure()
    plt.plot(path_data['heat_flux_amplitude_global_peak_path0'].t,path_data['heat_flux_amplitude_global_peak_path0'].values, label='raw', ls=':')

    if pulse >= 43543 and pulse <= 43621:
        t_scale_factor = 0.616342 / 0.56744
        plt.plot(path_data['heat_flux_amplitude_global_peak_path0'].t*t_scale_factor,path_data['heat_flux_amplitude_global_peak_path0'].values,
...
def compare_t2_t5_heat_flux():
    ...
    machine = 'mast_u'
    meta = dict(diag_tag_analysed=diag_tag_analysed.upper(), camera=diag_tag_analysed.upper(),
                pulse=pulse, machine=machine.upper(), signal=signal_ir)
    ...
    data = read_data_for_pulses_pickle(diag_tag_analysed, pulse, machine)
    path_data = data[pulse][0]['path_data']
    ...
    profile_t2 = profile_t2.where(t2_mask, drop=True)
    ...
    profile_t5 = profile_t5.where(t5_mask, drop=True)
    ...
    if plot_s_coord:
        ...
        elif align_signals == 'peak':
            ax.set_xlabel(r'$s-s_{strike point}$ [cm]')
        else:
            ax.set_xlabel(r'$s$ [m]')
    else:
        if plot_t2:
    ...
    alligned_str = '_'+align_signals if align_signals else ''
    log_y_str = '_logy' if log_y else ''
    x_coord_str = 's' if plot_s_coord else 'R'

    max_t2 = np.max(profile_t2.data)
    max_t5 = np.max(profile_t5.data)
    print(f'Max T2={max_t2:0.2f} MW, Max T5={max_t5:0.2f} MW')
    print(f'Delta q = {max_t2-max_t5: 0.2f}')
    print(f'Ratio q = {max_t2/max_t5: 0.2f}')

    fn = f'{diag_tag_analysed}_{pulse}_T2T5_vs_{x_coord_str}{alligned_str}{log_y_str}.png'
    path_fn = cwd / 'figures' / 'heat_flux_radial_profiles' / fn
    ...
    debug_plots.debug_plot_profile_2d(data_paths=path_data, param='heat_flux', ax=ax, robust=robust,
